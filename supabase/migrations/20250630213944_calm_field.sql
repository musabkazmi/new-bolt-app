/*
  ...
  2. Changes to menu_items
    - Add `required_inventory` (text array) to store required inventory items
...
-- Update menu items with required inventory
UPDATE menu_items
SET required_inventory = CASE
  WHEN name = 'Iced Coffee' THEN ARRAY['Coffee Beans', 'Milk', 'Ice']
  WHEN name = 'Iced Latte' THEN ARRAY['Coffee Beans', 'Milk', 'Ice']
  WHEN name = 'Lemon Iced Tea' THEN ARRAY['Lemons', 'Ice']
  WHEN name = 'Sparkling Water' THEN ARRAY['Soda Water', 'Lemons', 'Limes']
  WHEN name = 'Coca-Cola' THEN ARRAY['Ice', 'Lemons']
  WHEN name = 'Fresh Orange Juice' THEN ARRAY['Orange Juice', 'Ice']
  WHEN name = 'Mojito (Non-Alcoholic)' THEN ARRAY['Mint', 'Limes', 'Soda Water', 'Ice']
  WHEN name = 'Lemonade' THEN ARRAY['Lemons', 'Simple Syrup', 'Ice']
  WHEN name = 'Iced Chocolate' THEN ARRAY['Milk', 'Ice']
  WHEN name = 'Mango Smoothie' THEN ARRAY['Milk', 'Ice']
  WHEN name = 'House Wine' THEN ARRAY[]
  ELSE required_inventory
END
WHERE category IN ('Beverage', 'Drink', 'Alcohol', 'Coffee', 'Tea', 'Wine', 'Beer', 'Cocktail');

-- Create function to update menu item availability based on inventory
